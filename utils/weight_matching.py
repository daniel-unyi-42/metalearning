...
def mlp_permutation_spec(num_hidden_layers: int) -> PermutationSpec:
  ...
  return permutation_spec_from_axes_to_perm({
      "net.0.linear.weight": ("P_0", None),
      **{f"net.{i}.linear.weight": ( f"P_{i}", f"P_{i-1}")
         for i in range(1, num_hidden_layers)},
      **{f"net.{i}.linear.bias": (f"P_{i}", )
         for i in range(num_hidden_layers)},
      f"net.{num_hidden_layers}.weight": (None, f"P_{num_hidden_layers-1}"),
      f"net.{num_hidden_layers}.bias": (None, ),
  })
